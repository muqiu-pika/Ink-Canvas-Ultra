...
- 如需使用视频展台模式，请安装"视频展台"插件，安装后会在桌面生成快捷方式；也可使用命令行参数 `--video-presenter`
...
从 **2026/8/30** 开始，版本号统一采用 **年份 + 月份 + 当月第几个版本** 的三段式命名：
...
- `26.8.1` = 2026 年 8 月的第 1 个版本
- `26.8.2` = 2026 年 8 月的第 2 个版本
...
| 文件 | 字段 | 示例值 |
| --- | --- | --- |
| `AutomaticUpdateVersionControl.txt` | 全文 | `26.8.1` |
| `Ink Canvas/Properties/AssemblyInfo.cs` | `AssemblyVersion` / `AssemblyFileVersion` | `26.8.1.0`（末段固定为 0） |
| `Ink Canvas.csproj`（根目录，AnyCPU） | `ApplicationVersion` | `26.8.1.%2a` |
| `Ink Canvas/Ink Canvas.csproj`（x86） | `ApplicationVersion` | `26.8.1.%2a` |
| `Ink Canvas/app.manifest` | `assemblyIdentity version` | `26.8.1.0` |
| `Ink Canvas Ultra-AnyCPU.iss` | `MyAppVersion` | `26.8.1` |
| `README.md` | 当前版本 | `V26.8.1` |

> 安装包由 Inno Setup 脚本按 `Ink.Canvas.Ultra.V{版本}.Setup.exe` 命名产出
> （例如 `Ink.Canvas.Ultra.V26.8.1.Setup.exe`）。发布 Release 时，
> **tag 必须为 `v26.8.1`、安装包文件名必须完全一致**，否则自动更新下载会 404。
